...
export default function WhatsAppButton() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 1. Send data to our Lead Capture API (Pabbly + Sheets + HubSpot)
      await axios.post('/api/save-lead', {
        ...formData,
        loanType: 'WhatsApp Inquiry',
        source: 'WhatsApp_Floating_Button'
      });

      // 2. Open WhatsApp after successful capture
      ...
      // Fallback: Open WhatsApp anyway so we don't lose the customer
  ...
  return (
    <div className="wa-wrapper">
      {showForm && (
        <div className="wa-form-popup">
          <button className="close-btn" onClick={() => setShowForm(false)}>×</button>
          <h4>Quick Help 🚀</h4>
          <p>Enter details to start chat</p>
